...
function LoginContent() {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      toast.success("Welcome back to MIAKSAAA!");
      router.push(redirectUrl);
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden bg-[#0a0614] gradient-hero">
      ...
      <motion.div
        ...
        className="max-w-md w-full space-y-8 p-8 rounded-3xl relative z-10 glass border border-purple-500/25 shadow-2xl"
      >
        <div className="text-center">
          ...
          <p className="mt-2 text-sm" style={{ color: "var(--text-secondary)" }}>
            Access your premium luxury portal
          </p>
        </div>
        ...
        {error && (
          <motion.div
            ...
            className="p-3.5 rounded-xl flex items-start gap-2 text-sm bg-red-500/10 border border-red-500/30 text-red-300"
          >
        ...
        <form className="mt-8 space-y-4" onSubmit={handleEmailLogin}>
          ...
          <button
            ...
            className="btn-primary w-full py-3.5 text-sm gap-2 mt-6 cursor-pointer"
          >
        ...
        <button
          ...
          disabled={authLoading}
          className="w-full flex items-center justify-center gap-3 px-4 py-3 border rounded-xl font-semibold text-sm transition-all hover:bg-white/5 cursor-pointer"
        ...
        <div className="text-center mt-6">
          <p className="text-xs" style={{ color: "var(--text-secondary)" }}>
            New to MIAKSAAA?{" "}
            <Link href={`/register?redirect=${encodeURIComponent(redirectUrl)}`} className="text-amber-400 font-semibold hover:text-amber-300 transition-colors">
              Create an Account
            </Link>
          </p>
        </div>
